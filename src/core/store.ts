...
export class OmniStore {
  ...
  async getMany<T>(keys: Iterable<StorageKey>): Promise<BatchResult<Map<StorageKey, T | null>>> { const list = [...keys], data = new Map<StorageKey, T | null>(); const r = await Promise.allSettled(list.map(k => this.get<T>(k))); r.forEach((x, i) => { if (x.status === 'fulfilled') data.set(list[i]!, x.value) }); return { data, total: list.length, succeeded: data.size, failed: list.length - data.size } }
