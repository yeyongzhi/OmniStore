...
export class CookieDriver implements StorageDriver {
  ...
  async keys() { const prefix = `${this.namespace}:`; return (this.doc.cookie ? this.doc.cookie.split('; ') : []).map(p => decodeURIComponent(p.slice(0, p.indexOf('=')))).filter(k => k.startsWith(prefix)).map(k => k.slice(prefix.length)) }
